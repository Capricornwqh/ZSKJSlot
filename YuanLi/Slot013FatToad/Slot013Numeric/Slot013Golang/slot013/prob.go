...
type SlotProb struct {
	MGReelGroupRouletteMaps    [RTP_TOTAL]map[int]Roulette[int] // 主遊戲轉輪群組 Roulette Map
	FGReelGroupRouletteMaps    [RTP_TOTAL]map[int]Roulette[int] // 免費遊戲轉輪群組 Roulette Map
	MGCoverScatterRouletteMaps [RTP_TOTAL]map[int]Roulette[int] // 主遊戲金蟾铜钱 Roulette Map
}
...
func (slot *SlotProb) SetRTPRoulette() {
	for rtp := range RTP_TOTAL {
		...
		rouletteMap := make(map[int]Roulette[int])
		for buyType, groupWT := range MGReelGroupWT[rtp] {
			roulette := NewRouletteFromList(groupWT)
...
func (slot *SlotProb) Run(rtp int, lineBet int, result *SlotResult, debugCmdList []DebugCmd) int {
	result.GameMode = GAME_MODE_NORMAL
	...
	// 如果赢分=0 且没有金蟾，触发天降横财
	if buyType == BUY_NONE && result.MainWin <= 0 && len(result.WWWild) <= 0 &&
	...
	case BUY_NONE:
		result.FGIndex = RandChoiceByWeight(FGIndexList, FGIndexWT)
	case BUY_FREE_SPINS:
		result.FGIndex = RandChoiceByWeight(FGIndexList, FGBuyFreeIndexWT)
	case BUY_SUPER_FREE_SPINS:
		result.FGIndex = RandChoiceByWeight(FGIndexList, FGBuySuperIndexWT)
	default:
	}
	...
	for result.FGSpinCount > 0 {
		result.GameMode = GAME_MODE_FREE
		result.FGSpinCount--
		tmpSpinCount++
...
func (slot *SlotProb) getMainWildMulti(buyType, index int) Symbol {
	...
	case BUY_NONE:
		if index == MAIN_GAME_01 && rand.IntN(100) < SSWILD_MAIN_GAME_1 {
			return RandChoiceByWeight(MGSSMultiList, MGSSMultiWT)
		} else if index == MAIN_GAME_02 && rand.IntN(100) < SSWILD_MAIN_GAME_2 {
			return RandChoiceByWeight(MGSSMultiList, MGSSMultiWT)
		}
	case BUY_FREE_SPINS:
		if rand.IntN(100) < SSWILD_BUY_FREE {
			return RandChoiceByWeight(MGSSMultiList, MGBuySSMultiWT)
		}
	case BUY_SUPER_FREE_SPINS:
...
func (slot *SlotProb) getFreeWildMulti(buyType, index int) Symbol {
	...
	case BUY_NONE:
		if index == FREE_INDEX_2 && rand.IntN(100) < SSWILD_FREE_GAME_2 {
			return RandChoiceByWeight(FGSSMultiList, FGSSMultiWT)
		}
	case BUY_FREE_SPINS:
...
func (slot *SlotProb) WWLevelUp(result *SlotResult) {
	...
	*/
	...
	if result.GameMode == GAME_MODE_FREE {
...
func (slot *SlotProb) RandMGSymbol(rtp int, buyType int, lineBet int, tumbleResult *TumbleResult, debugCmd *DebugCmd) (int, int, []*WildStruct, []*WildStruct) {
	...
	// 处理铜钱倍数
	if len(tmpSSWild) > 0 {
		if buyType == BUY_SUPER_FREE_SPINS {
			// Buy Super 的时候，从铜钱wild里面随机选一个变成倍数
			randIndex := rand.IntN(len(tmpSSWild))
			tmpSSWild[randIndex].WildSymbol = RandChoiceByWeight(MGSSMultiList, MGSSMultiWT)
	...
	return groupIdx, ERROR_CODE_OK, tmpWWWild, tmpSSWild
}
...
func (slot *SlotProb) RandFGSymbol(rtp int, buyType int, lineBet int, spinResult *SpinResult, result *SlotResult, debugCmd *DebugCmd) (int, int) {
	groupIdx := RandChoiceByWeight(FGGroupIndexList, FGLevelGroupIndexWT[result.BuyType][result.FGIndex][result.WWLevel])
