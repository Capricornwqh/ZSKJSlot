...
func CorrectThrNum() int {
	if utils.Conf.Environment == "dev" {
		return 1
	} else if utils.Conf.Game.MCCount < 1 {
		return runtime.GOMAXPROCS(0)
	}
	return utils.Conf.Game.MTCount
}
...
func MonteCarlo(ctx context.Context, s Stater, g SlotGame, reels Reels) {
	s.SetPlan(utils.Conf.Game.MCCount * 1e6)
	var tn = CorrectThrNum()
	var tn64 = uint64(tn)
...
func MonteCarloPrec(ctx context.Context, s Stater, g SlotGame, reels Reels, calc func(io.Writer) float64) {
	...
	var rtpnum uint64
	var mcc = utils.Conf.Game.MCCount * 1e6 / CtxGranulation
	if mcc == 0 {
		...
		go func() {
			...
			for atomic.LoadUint64(&rtpnum) < mcc {
				...
				if reshuf%CtxGranulation == 0 {
					...
					var rtp = calc(io.Discard)
					rtpmux.Lock()
					if diff := rtp - rtpcmp; diff < utils.Conf.Game.MCPrec && diff > -utils.Conf.Game.MCPrec {
						rtpnum++
					} else {
						rtpnum = 0
						rtpcmp = rtp
...
func ScanReels(ctx context.Context, s Stater, g SlotGame, reels Reels,
	...
	if utils.Conf.Game.MCPrec > 0 {
		montecarloprec(ctx2, s, g, reels)
	} else if utils.Conf.Game.MCCount > 0 {
